...
-- Async counterpart of fetch_single_chapter_source. Fetches a chapter's content
-- shards (and the style shard) concurrently via Client:request_async. on_complete
-- receives the raw xhtml string; on_error receives a message. Falls back to the
-- synchronous path when the chapter reader state (psvts) is not yet available.
function Content.fetch_single_chapter_source_async(client, settings, book, chapter, state, on_complete, on_error)
    if not book.psvts then
        local ok, x = pcall(Content.fetch_single_chapter_source, client, settings, book, chapter, state)
        if ok then
            on_complete(x)
        else
            on_error(tostring(x))
    ...
    local chapter_url = WeRead.reader_url(book_id, chapter_uid)
    local waiting = 0
    local finished = false
    local results = {}
    local xhtml = nil

    local function finish(err, val)
        if finished then
            return
        end
        finished = true
    ...
    local function try_build()
        if finished or xhtml ~= nil then
            return
        end
        if book._content_format == "txt" then
            if results.t0 then
                xhtml = Content.txt_to_xhtml(Content.decode_content_shards(results.t0, results.t1 or "", ""))
            end
        elseif results.e0 and results.e1 and results.e3 then
            xhtml = Content.decode_content_shards(results.e0, results.e1, results.e3)
        end
    end

    local function note_done()
        if finished then
            return
        end
        try_build()
        if xhtml ~= nil and results.css ~= nil then
            finish(nil, xhtml)
        end
    end

    local function start_one(endpoint, store_key)
        waiting = waiting + 1
        local is_style = endpoint:find("/e_2", 1, true) ~= nil
        local params = WeRead.make_content_params(book_id, chapter_uid, book.psvts, {
            sc = 1,
            style = is_style,
        })
        client:request_async({
            ...
            waiting = waiting - 1
            if not code or code < 200 or code >= 300 then
                return finish(endpoint .. " failed: HTTP " .. tostring(code))
            end
            if text == "{}" then
                return finish(endpoint .. " returned empty object")
            end
            results[store_key] = text
            note_done()
        end, function(errmsg)
            waiting = waiting - 1
            finish(endpoint .. ": " .. tostring(errmsg))
        end)
    end

    -- Style shard (e_2) is fetched concurrently with the content shards.
    start_one("/web/book/chapter/e_2", "css")

    if book._content_format == "txt" then
        start_one("/web/book/chapter/t_0", "t0")
        start_one("/web/book/chapter/t_1", "t1")
    else
        -- Fetch e_0 first; its payload decides epub vs txt format, then we
        -- fan out to the remaining shards.
        waiting = waiting + 1
        local params = WeRead.make_content_params(book_id, chapter_uid, book.psvts, {
            sc = 1,
            style = false,
        ...
        client:request_async({
            url = "https://weread.qq.com/web/book/chapter/e_0",
            method = "POST",
            headers = {
                ["Content-Type"] = "application/json;charset=UTF-8",
                ["Origin"] = "https://weread.qq.com",
            ...
            waiting = waiting - 1
            if not code or code < 200 or code >= 300 then
                return finish("e_0 failed: HTTP " .. tostring(code))
            end
            if text == "{}" then
                return finish("e_0 returned empty object")
            end
            results.e0 = text
            if text:sub(1, 1) == "{" and text:find('"bookId"', 1, true) then
                book._content_format = "txt"
                start_one("/web/book/chapter/t_0", "t0")
                start_one("/web/book/chapter/t_1", "t1")
            else
                book._content_format = "epub"
                start_one("/web/book/chapter/e_1", "e1")
                start_one("/web/book/chapter/e_3", "e3")
            end
            note_done()
        end, function(errmsg)
            waiting = waiting - 1
            finish("e_0: " .. tostring(errmsg))
        end)
    end
end
