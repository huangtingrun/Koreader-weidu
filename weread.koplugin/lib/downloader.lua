...
local AsyncHttp = require("lib.async_http")
...
function Downloader:start(book, chapters, suffix, options)
    ...
    self.run_online_task(task_label, function()
        ...
        if options.download_book_images ~= nil or options.download_underlines_and_thoughts ~= nil
            or options.download_async ~= nil or options.download_concurrency ~= nil then
            dl._cache_original = {}
            for k, v in pairs(cache) do dl._cache_original[k] = v end
            if options.download_book_images ~= nil then
                cache.download_book_images = options.download_book_images
            end
            if options.download_underlines_and_thoughts ~= nil then
                cache.download_underlines_and_thoughts = options.download_underlines_and_thoughts
            end
            if options.download_async ~= nil then
                cache.download_async = options.download_async
            end
            if options.download_concurrency ~= nil then
                cache.download_concurrency = options.download_concurrency
            end
            self.settings:set("cache", cache)
            self.settings:flush()
        end

        if not dl.single_chapter and cache.download_async and AsyncHttp.supported() then
            self:_runConcurrent(dl)
        else
...
-- Build the EPUB from the collected chapter bodies/assets and present the
-- completion dialog. Shared by the sequential _step path and the concurrent path.
function Downloader:_finishBuild(dl)
    if dl._cache_original then
        local cache = self.settings:get("cache")
        for k, v in pairs(dl._cache_original) do
            cache[k] = v
        end
        self.settings:set("cache", cache)
        self.settings:flush()
        dl._cache_original = nil
    end
    ...
    if dl.annotation_failed_batches and dl.annotation_failed_batches > 0 then
    ...
    self:_perf(dl, "download_total", dl.started_at,
        ...
        "failed_thought_batches=", tostring(dl.annotation_failed_batches or 0))
...
-- Concurrent bulk download: launch up to `cache.download_concurrency` chapter
-- source fetches in parallel via Client:request_async, then finalize serially.
function Downloader:_runConcurrent(dl)
    local cache = self.settings:get("cache")
    local concurrency = tonumber(cache.download_concurrency) or 4
    if concurrency < 1 then concurrency = 1 end

    dl.in_flight = 0
    dl.source_done = 0
    dl.bodies = dl.bodies or {}
    dl.assets = dl.assets or {}
    dl.selected = dl.selected or {}
    dl.state = dl.state or {}
    dl.failed = dl.failed or {}
    dl.annotation_failed_batches = 0
    dl._concurrent_finalized = false

    self:_setStage(dl, T(_("Downloading (parallel) %1/%2"), "0", tostring(dl.total)), 0)

    local function maybeFinish()
        if dl.source_done >= dl.total and dl.in_flight == 0 and not dl._concurrent_finalized then
            dl._concurrent_finalized = true
            self:_finalizeConcurrent(dl)
        end
    end
    ...
    local function launchMore()
        ...
        while dl.index <= dl.total and dl.in_flight < concurrency do
            local chapter = dl.chapters[dl.index]
            dl.index = dl.index + 1
            dl.in_flight = dl.in_flight + 1
            self:_setStage(dl,
                T(_("Downloading (parallel) %1/%2"),
                    tostring(dl.source_done + dl.in_flight), tostring(dl.total)),
                dl.source_done + dl.in_flight - 0.5)
            Content.fetch_single_chapter_source_async(self.client, self.settings, dl.book, chapter, dl.state,
                function(xhtml)
                    local uid = tostring(chapter.chapterUid or dl.index)
                    dl.bodies[uid] = xhtml
                    table.insert(dl.selected, chapter)
                    dl.in_flight = dl.in_flight - 1
                    dl.source_done = dl.source_done + 1
                    self:_perf(dl, "chapter_source_async", dl.started_at, "uid=", uid)
                    launchMore()
                end,
                function(errmsg)
                    local uid = tostring(chapter and chapter.chapterUid or dl.index)
                    table.insert(dl.failed, uid)
                    logger.warn(LOG_MODULE, "async chapter failed:", uid, errmsg)
                    dl.in_flight = dl.in_flight - 1
                    dl.source_done = dl.source_done + 1
                    launchMore()
                end)
...
-- Serial pass after all sources are in: finalize each chapter (extract images),
-- apply annotations if enabled, then build the EPUB. Kept serial on purpose so
-- blocking network/CPU work never stalls the async socket pump.
function Downloader:_finalizeConcurrent(dl)
    local i = 0
    local total = #dl.selected
    local function step()
        i = i + 1
        if i > total then
            self:_finishBuild(dl)
        ...
        local chapter = dl.selected[i]
        local uid = tostring(chapter.chapterUid or i)
        local xhtml = dl.bodies[uid]
        self:_setStage(dl, T(_("Processing chapter %1/%2"), tostring(i), tostring(total)), i - 0.1)
        local ok, fxhtml, chapter_assets = pcall(function()
            return Content.finalize_single_chapter_content(self.client, self.settings, dl.book, chapter, xhtml, dl.state)
        end)
        if not ok then
            logger.warn(LOG_MODULE, "finalize failed, skipping chapter:", uid)
            table.insert(dl.failed, uid)
            UIManager:scheduleIn(0, step)
            return
        end
        dl.bodies[uid] = fxhtml
        for _, a in ipairs(chapter_assets or {}) do
            table.insert(dl.assets, a)
        end
        if Thoughts.is_download_enabled(self.settings) then
            local ok2, underlines, ranges = pcall(Thoughts.fetch_underlines,
                self.client, self.settings, dl.book.book_id or dl.book.bookId, chapter.chapterUid)
            if ok2 and type(underlines) == "table" then
                local processed, annotation_css = Thoughts.apply_data(self.settings,
                    dl.book.book_id or dl.book.bookId, chapter.chapterUid, fxhtml, underlines, {}, dl.book,
                    { rebuild_thought_db = (i == 1) })
                dl.bodies[uid] = processed
