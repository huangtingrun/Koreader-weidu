...
function Settings:new()
    ...
    if cache.download_async == nil then
        cache.download_async = false
        cache_changed = true
    end
    if cache.download_concurrency == nil or tonumber(cache.download_concurrency) < 1 then
        cache.download_concurrency = 4
        cache_changed = true
    end
    ...
    local shelf = obj.store:readSetting("shelf", deepcopy(defaults.shelf))
    if shelf.show_covers == nil then
        shelf.show_covers = true
        shelf_changed = true
    end
    if shelf.sort_order == nil or shelf.sort_order == "" then
        shelf.sort_order = "time_desc"
        shelf_changed = true
    end
    if shelf_changed then
        obj.store:saveSetting("shelf", shelf)
        obj.store:flush()
    end
