...
-- Build the LTN12 request table (headers, cookie injection, timeout, method)
-- shared by both the blocking Client:request and the async Client:request_async.
-- Returns the req_opts table with internal fields prefixed by "_".
function Client:_build_req_opts(opts)
    opts = opts or {}
    ...
    req_opts._block_timeout = block_timeout
    req_opts._total_timeout = total_timeout
    req_opts._is_handle_cookie = is_handle_cookie
    return req_opts
end

function Client:request(opts)
    opts = opts or {}
    local req_opts = self:_build_req_opts(opts)
    local block_timeout = req_opts._block_timeout
    local total_timeout = req_opts._total_timeout
    ...
    if req_opts._is_handle_cookie and opts.persist_response_cookies ~= false then
...
-- Non-blocking variant used by the bulk downloader to fetch chapters in parallel.
-- Falls back to the synchronous Client:request (on a UIManager tick) when the async
-- transport is unavailable, so callers can treat it uniformly.
function Client:request_async(opts, on_complete, on_error)
    if not AsyncHttp.supported() then
        UIManager:scheduleIn(0, function()
            ...
            if ok then
                if on_complete then
                    on_complete(code, res, headers, status)
                end
            else
                if on_error then
                    on_error(tostring(res))
                end
            end
        end)
        return
    ...
    local req_opts = self:_build_req_opts(opts)
    AsyncHttp.submit(self, opts, req_opts, on_complete, on_error)
end
